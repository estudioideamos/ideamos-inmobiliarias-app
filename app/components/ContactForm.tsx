...
export default function ContactForm({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <form className={`contact-form${compact ? " contact-form-compact" : ""}`} onSubmit={sendByEmail} data-email-endpoint={contactEmail}>
      ...
      <div className="contact-form-submit"><p className={`form-status form-status-${status}`} aria-live="polite"><i /> {status === "sent" ? "Consulta enviada. Te vamos a responder a la brevedad." : status === "error" ? "No pudimos enviarla. Escribinos a hola@ideamos.com.ar." : "Tus datos se envían de forma segura al email de nuestro equipo."}</p><button className="button button-accent" type="submit" disabled={status === "sending"}>{status === "sending" ? "Enviando…" : "Enviar consulta"} <span aria-hidden="true">↗</span></button></div>
    </form>
  );
}
